...
class Utils(object):
    ...
    def __init__(self,config):
        ...
        self.min_congestion_duration = config.min_congestion_duration  # Minimum duration for congestion
        self.max_congestion_duration = config.max_congestion_duration  # Maximum duration for congestion
    ...
    def get_node_mean_congestion(self,node_id,source_edge, source_edge_congestion):
        congestion=0
        count=0
        out_edges=self.network.get_out_edges(node_id).copy()
        if '-' in source_edge:
            out_edges.remove(source_edge[1:])
        else:
            out_edges.remove('-'+source_edge)
        for edge in out_edges:
            if self.network.edge_speed_dic[edge]['is_congested']:
                congestion+=1
            count+=1
        congestion+=source_edge_congestion
        count+=1
        return congestion/count
    
    def get_node_mean_congestion_exception(self,node_id,source_node, source_edge_congestion):
        congestion=0
        count=0
        out_edges=self.network.get_out_edges(node_id).copy()
        for edge in out_edges:
            if self.network.get_edge_head_node(edge)==source_node:
                continue
            if self.network.edge_speed_dic[edge]['is_congested']:
                congestion+=1
            count+=1
    ...
    def get_state(self,source_edge,source_node,sink_node):
        ...
        source_out_edges = self.network.get_out_edges(source_node).copy()
        if '-' in source_edge:
            source_out_edges.remove(source_edge[1:])
        else:
            source_out_edges.remove('-'+source_edge)
        neighbor_nodes = [self.network.get_edge_head_node(edge) for edge in source_out_edges]
        neighbour_dic={}
        for neighbor_node in neighbor_nodes:
            if neighbor_node not in self.agent_index_dic:
                source_neighbor_in_edges = self.network.get_in_edges(neighbor_node)
                for edge in source_neighbor_in_edges:
                    if self.network.get_edge_tail_node(edge)!=source_node:
                        neighbor_nodes.remove(neighbor_node)
                        new_neighbor_node=self.network.get_edge_tail_node(edge)
                        neighbor_nodes.append(new_neighbor_node)
                        for edge in source_neighbor_in_edges:
                            if self.network.get_edge_tail_node(edge)==source_node:
                                if self.network.edge_speed_dic[edge]['is_congested']:
                                    neighbour_dic[new_neighbor_node]=self.get_node_mean_congestion_exception(new_neighbor_node,neighbor_node,1)
                                else:
                                    neighbour_dic[new_neighbor_node]=self.get_node_mean_congestion_exception(new_neighbor_node,neighbor_node,0)
        ...
        for neighbor_node in neighbor_nodes:
            ...
            for edge in source_neighbor_in_edges:
                if self.network.get_edge_tail_node(edge)==source_node:
                    if self.network.edge_speed_dic[edge]['is_congested']:
                        neighbour_dic[neighbor_node]=self.get_node_mean_congestion(neighbor_node,edge,1)
                    else:
                        neighbour_dic[neighbor_node]=self.get_node_mean_congestion(neighbor_node,edge,0)
        neighbor_nodes_idx = [self.agent_index_dic[node] for node in neighbor_nodes]
        #replace the keys with their idxs in neighbour_dic
        new_neighbour_dic={}
        for node in neighbour_dic:
            new_neighbour_dic[self.agent_index_dic[node]]=neighbour_dic[node]
        neighbour_dic=new_neighbour_dic
        #print("neighbour_dic: ", neighbour_dic)
        ...
        return {
                ...
                "destination_aware_network_state": destination_aware_network_state,
                "neighbor_nodes_idx": neighbor_nodes_idx,
                "neighbour_dic": neighbour_dic
                }
    ...
    def propagate_congestion(self, edge, depth=2, probability=0.5):
        if depth == 0:
            return
        headnode=self.network.get_edge_head_node(edge)
        tailnode=self.network.get_edge_tail_node(edge)
        connected_edges = set(self.network.get_out_edges(headnode) + self.network.get_in_edges(tailnode))
        for connected_edge in connected_edges:
            if self.seeded_random_generator.random() < probability:
                self.config.edge_congestion_duration[connected_edge] = self.seeded_random_generator.randint(self.min_congestion_duration, self.max_congestion_duration)
                self.network.edge_speed_dic[connected_edge]['is_congested'] = True
                free_flow_speed = self.network.edge_speed_dic[connected_edge]['speed']
                traci.edge.setMaxSpeed(connected_edge, free_flow_speed * 0.399)
                for agent_id in self.agent_path_dic:
                    out_edges_list = list(self.agent_path_dic[agent_id])
                    for edge_number, out_edge in enumerate(out_edges_list):
                        if connected_edge in self.agent_path_dic[agent_id][out_edge]:
                            self.config.network_state[self.agent_index_dic[agent_id]][edge_number] = 1
                            break
                self.config.congested_edges.append(connected_edge)
                self.propagate_congestion(connected_edge, depth - 1, probability * 0.5)

    
    def set_network_state(self, time):
        for agent_id in self.agent_path_dic:
            out_edges_list = list(self.agent_path_dic[agent_id])
            for edge_number in range(len(out_edges_list)):
                path_key = out_edges_list[edge_number]
                for edge in self.agent_path_dic[agent_id][path_key]:
                    if edge not in self.config.edge_congestion_duration:
                        self.config.edge_congestion_duration[edge] = 0
                    else:
                        self.config.edge_congestion_duration[edge] -= 1
                        if self.config.edge_congestion_duration[edge] <= 0:
                            self.config.edge_congestion_duration[edge] = 0
                            if edge in self.config.congested_edges:
                                self.config.congested_edges.remove(edge)
                    ...
                    free_flow_speed = self.network.edge_speed_dic[edge]['speed']
                    #print(f"free flow speed vs average speed: {free_flow_speed} vs {average_speed}")
                    #print("travel time: ", travel_time, "congestion threshold: ", congestion_threshold)
                    if average_speed <= 0.4 * free_flow_speed:
                        self.config.network_state[self.agent_index_dic[agent_id]][edge_number] = 1
                        #traci.edge.setMaxSpeed(edge, free_flow_speed * self.config.congestion_speed_factor)
                        self.network.edge_speed_dic[edge]['is_congested'] = True
                    else:
                        self.config.network_state[self.agent_index_dic[agent_id]][edge_number] = 0
                        #traci.edge.setMaxSpeed(edge, free_flow_speed)
                        self.network.edge_speed_dic[edge]['is_congested'] = False
                        
                    if time%self.config.traffic_period==100:
                        if self.seeded_random_generator.random() < self.config.congestion_epsilon:
                            self.config.edge_congestion_duration[edge] = self.seeded_random_generator.randint(self.min_congestion_duration, self.max_congestion_duration)
                            self.config.network_state[self.agent_index_dic[agent_id]][edge_number] = 1
                            free_flow_speed = self.network.edge_speed_dic[edge]['speed']
                            traci.edge.setMaxSpeed(edge, free_flow_speed * 0.399)
                            self.network.edge_speed_dic[edge]['is_congested'] = True
                            self.config.congested_edges.append(edge)
                            self.propagate_congestion(edge, 2, 0.25 * self.config.congestion_epsilon)
                        


    def reset_congestion_duration(self):
        for edge in self.config.edge_congestion_duration:
            self.config.edge_congestion_duration[edge] = 0
            traci.edge.setMaxSpeed(edge, self.network.edge_speed_dic[edge]['speed'])
        #clear congested edges
        self.config.congested_edges = []
