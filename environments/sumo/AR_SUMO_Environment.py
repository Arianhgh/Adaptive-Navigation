...
class AR_SUMO_Environment(gym.Env):
		...
		def reset(self,episode,config):
			...
			self.refresh_exp()
			self.utils.reset_congestion_duration()
...
#-------------------------------------------------------------
			...
			#for each edge, decrement the congestion duration
		...
		def change_traffic_condition(self,time):
			self.utils.set_network_state(time)
			
			if self.should_insert_new_vc(time):
				if time%self.config.uniform_demand_period==0:
		...
		def log_data(self):
			if self.cummulative_n_success_v!=0:
				...
				print("AVTT:", self.cummulative_tt/self.cummulative_n_success_v)
				print("success:", self.cummulative_n_success_v)
