...
routing_modes=["Q_routing_2_hop","Q_routing_1_hop","Q_routing_0_hop","Q_routing","TTSPWRR","TTSP","meanfield"]
...
config.does_need_network_state=config.routing_mode in ["Q_routing_2_hop","Q_routing_1_hop","Q_routing_0_hop","meanfield"]
config.does_need_network_state_embeding=config.routing_mode in ["Q_routing_2_hop","Q_routing_1_hop","meanfield"]
config.meanfield = config.routing_mode == "meanfield"
...
config.Constants = {
    ...
    "SUMO_PATH" : r"C:\Program Files (x86)\Eclipse\Sumo", #path to sumo in your system
    "SUMO_GUI_PATH" : r"C:\Program Files (x86)\Eclipse\Sumo\bin\sumo-gui", #path to sumo-gui bin in your system
    "SUMO_SHELL_PATH": r"C:\Program Files (x86)\Eclipse\Sumo\bin\sumo",
...
config.uniform_demand_period=2
config.biased_demand_2_uniform_demand_ratio=0.1

config.traffic_period=500
config.max_num_sim_time_step_per_episode=2000

config.demand_scale=1
config.congestion_epsilon=0.2
config.congestion_speed_factor=0.1
config.max_congestion_duration = 250
config.min_congestion_duration = 100
config.edge_congestion_duration = {}  # Track congestion duration for edges
config.congested_edges = []  # Track congested edges
...
config.model_version="June-2024"
...
if config.routing_mode=="meanfield":
    num_GAT_layers=1
    config.network_embed_size=9
    num_GAT_features_per_layer=[2,4]
    DQN_linear_hidden_units=[15,7]
