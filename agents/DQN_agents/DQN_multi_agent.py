...
class DQN(Base_Agent):
    ...
    def pick_action(self, states):
        ...
        if self.config.does_need_network_state and not self.config.meanfield:
        ...
        for state,network_state_embeding in zip(states,network_state_embedings):
            ...
            if self.config.meanfield:
                intersection_state_embeding=[]
                for neighbour_idx in state['neighbour_dic']:
                    neighbor_congestion=state['neighbour_dic'][neighbour_idx]
                    neighbor_distance=state['destination_aware_network_state'][neighbour_idx][4]
                    intersection_state_embeding.extend([neighbour_idx, neighbor_congestion, neighbor_distance])
                while len(intersection_state_embeding) < 9:
                    intersection_state_embeding.append(0.0)

                intersection_state_embeding = intersection_state_embeding[:9]
                intersection_state_embeding = torch.tensor(intersection_state_embeding, dtype=torch.float32).to(self.device)
    ...
    def compute_q_values_for_next_states(self, next_states,dones):
        ...
        if self.config.does_need_network_state:
            if self.config.does_need_network_state_embeding:
                if self.config.meanfield:
                    network_state_embeding_batch=[]
                    for state in next_states:
                        if state==None:
                            network_state_embeding_batch.append(torch.zeros(9))
                            continue
                        intersection_state_embeding=[]
                        for neighbour_idx in state['neighbour_dic']:
                            neighbor_congestion=state['neighbour_dic'][neighbour_idx]
                        ...
                        while len(intersection_state_embeding) < 9:
                            intersection_state_embeding.append(0.0)
                        intersection_state_embeding = intersection_state_embeding[:9]  # Ensure only 9 elements if there are more than 3 neighbors
                        intersection_state_embeding = torch.tensor(intersection_state_embeding, dtype=torch.float32)
                        network_state_embeding_batch.append(intersection_state_embeding)
                    network_state_embeding_batch=torch.vstack(network_state_embeding_batch).to(self.device)
                else:
        ...
        for agent_id in masks_dic:
            ...
            if self.config.meanfield:
                intersec_states_embeding=network_state_embeding_batch[masks_dic[agent_id]["batch_indexs"]]
            else:
